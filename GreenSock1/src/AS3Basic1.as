package
{
	...
	public class AS3Basic1 extends Sprite 
	{
		...
		public function AS3Basic1() 
		{
			...
			// ＊注＊ 次のようなint型を指定するかのような書き方をする場合があるが、型指定はできません。
			// var sample:Array/*int*/ = [];
			// 「/*int*/」はコメントとして書かれているので、コンパイラは無視する。しかしコードエディタの一部は型指定があるかのように補完を行うので、使われる。
		...
		// 関数にプリミティブ（int,uint,Number,Boolean,String）を渡す場合には、値渡しになります。
		...
		private function setTrace(... arguments):void {
			...
			// trace()ではprintf("a = %d",1)のような変換ができません。渡した値を出力するのみの機能です。
